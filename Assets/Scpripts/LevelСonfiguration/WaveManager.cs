...
// ”правл€ет волнами врагов: спавн, отслеживание, завершение. јвтоматический переход на бесконечный режим, если не задана WaweData - тоже сразу переключаемс€ на бесконечный
public class WaveManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("WaveManager: ќтсутствует spawnIndicatorPrefab!");
            return;
        }
    ...
    public void StartNextWave()
    {
        if (isWaveActive)
        {
            Debug.LogWarning("WaveManager: ¬олна уже активна!");
            return;
        }
        ...
        if (waveData != null && waveData.Waves.Count > 0 && CurrentWaveIndex < waveData.Waves.Count)
        {
            ...
            Debug.Log($"¬олна {CurrentWaveIndex + 1} началась (обычна€)!");
        }
        else
        {
            if (infiniteData == null)
            ...
            Debug.Log($"¬олна {CurrentWaveIndex + 1} началась (бесконечна€)!");
        }
    }

    // ”станавливает индекс волны
    ...
    public void ShowNextWaveSpawnIndicator()
    {
        ...
        {
            Debug.LogWarning("WaveManager: ќтсутствует spawnIndicator!");
            return;
        }
    ...
    private IEnumerator SpawnWave(Wave wave)
    {
        ...
        foreach (var config in wave.Enemies)
        {
            for (int i = 0; i < config.Count; i++)
            {
                ...
                // ѕоказываем индикатор перед спавном.
                ShowSpawnIndicator(spawnPos);
    ...
    // —павнит врагов бесконечной волны
    private IEnumerator SpawnInfiniteWave(int waveNumber)
    {
        ...
        foreach (var template in infiniteData.Enemies)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                ShowSpawnIndicator(spawnPos); // ѕоказываем индикатор перед спавном.
    ...
    // ѕолучает точки спавна дл€ обычной волны
    ...
    // ѕолучает точки спавна дл€ бесконечной волны
    ...
    // —оздаЄт фронт спавна и возвращает его точки
    ...
    // «авершает волну, начисл€ет награду, сохран€ет игру
